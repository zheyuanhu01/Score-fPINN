...
def sample_test_points():
    ...
    x = ND_stable_distribution(alpha, batch_size, d) * (1 - np.exp(-t.reshape(-1, 1))) ** (1 / args.alpha) + \
        x0 * np.exp(-t.reshape(-1, 1) / args.alpha)
    return x, t
...
class PINN:
    ...
    def resample(self, A, N, rng):
        ...
        x0 = jax.random.normal(keys[0], shape=(N, self.dim)) * jnp.sqrt(Gamma).reshape(1, self.dim)
        ...
        xf = xf * ((1 - jnp.exp(-tf).reshape(-1, 1)) ** (1 / self.alpha)) + \
            x0 * jnp.exp(-tf / self.alpha).reshape(-1, 1)
    ...
    def score_fpinn(self, params, x, t):
        A = lambda x: -x / self.alpha - self.net_S1.apply(params, x, t)
    ...
    #     residual = s + x0 / Gamma / jnp.exp(-1 * t / self.alpha)
    #     return residual
    ...
    def step2(self, params, opt_state, rng, A):
        _, xf, tf, rng = self.resample(A, self.N_SM, rng)
    ...
    def residual_pred_ll(self, params, x, t):
        ...
        # q_x = self.net_S2.apply(self.params_S2, x, t)
        pred_x = jnp.dot(self.net_S1.apply(self.params_S1, x, t) + x / self.alpha, q_x) + \
            jnp.mean(jnp.diag(jax.jacrev(self.net_S1.apply, argnums=1)(self.params_S1, x, t))) + 1 / self.alpha
